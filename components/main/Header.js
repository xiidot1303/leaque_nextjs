import {useState} from "react";

const navigation = [
  { id: 1, title: 'P2P', path: '/profile/p2p', className: 'header__link' },
  { id: 2, title: 'Staking', path: '/profile/invest', className: 'header__link' },
  {
    id: 3,
    title: 'Exchange',
    path: '/profile/swap',
    className: 'header__link hot',
  },
  { id: 4, title: 'Wallet', path: '/profile/wallet', className: 'header__link' },
];

const sign = [
  { id: 1, title: 'Log In', path: '/signin', className: 'header__link-log' },
  { id: 2, title: 'Sign Up', path: '/signup', className: 'header__link-up' },
];

const Header = () => {
  const [wrapperActive, setWrapperActive] = useState(false)

  function btnOnclick() {
    if (wrapperActive) {
      setWrapperActive(false)
    } else setWrapperActive(true)
  }

  return (
    <header className="header">
      <div className="header__container">
        <a className="header__logo" href="#">
          <img style={{ width: '33px' }} src="img/default.png" alt="" />
          Leaque{' '}
        </a>
        <div className={ wrapperActive ? "header__wrapper header__wrapper-active" : 'header__wrapper'}>
          <div className="header__center">
            {navigation.map(({ id, title, path, className }) => (
              <a key={id} href={path} className={className}>
                {title}
              </a>
            ))}
          </div>
          <div className="header__right">
            {sign.map(({ id, title, path, className }) => (
              <a key={id} href={path} className={className}>
                {title}
              </a>
            ))}
          </div>
        </div>
        <div className="header__btn-mobile" onClick={btnOnclick}>
          <svg width="35" height="35" viewBox="0 0 100 100">
            <path
              className="header__nav-line header__nav-line1"
              d="M 20,29.000046 H 80.000231 C 80.000231,29.000046 94.498839,28.817352 94.532987,66.711331 94.543142,77.980673 90.966081,81.670246 85.259173,81.668997 79.552261,81.667751 75.000211,74.999942 75.000211,74.999942 L 25.000021,25.000058"
            />
            <path className="header__nav-line header__nav-line2" d="M 20,50 H 80" />
            <path
              className="header__nav-line header__nav-line3"
              d="M 20,70.999954 H 80.000231 C 80.000231,70.999954 94.498839,71.182648 94.532987,33.288669 94.543142,22.019327 90.966081,18.329754 85.259173,18.331003 79.552261,18.332249 75.000211,25.000058 75.000211,25.000058 L 25.000021,74.999942"
            />
          </svg>
        </div>
      </div>
    </header>
  );
};

export default Header;
